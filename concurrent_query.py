...
conn.autocommit = True  # so updates commit immediately

# Create a cursor factory helper
def get_cursor():
    return conn.cursor()

if not os.path.exists("Images"):
    os.makedirs("Images")
# Assuming your CockroachDB table "user_review" has these columns:
# rating (int), asin (string), parent_asin (string), verified_purchase (bool), helpful_vote (int),
...
for count in concurrent_counts:
    ...
    with ThreadPoolExecutor(max_workers=count) as executor:
        # Run the first 'count' queries concurrently
...
plt.title("Concurrent Queries vs Response Time (CockroachDB)")
plt.grid(True)
plt.legend()
plt.tight_layout()
try:
    plt.savefig("Images/concurrent_queries_vs_time_cockroachdb.png", dpi=150)
except:
    pass
plt.show()
